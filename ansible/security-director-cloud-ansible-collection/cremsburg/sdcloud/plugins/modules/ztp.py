...
def manage_device(resources, module, rest):
    ...
    for each in resources["device-view"]:
        if 'serial_number' in each['system_info']:
            if each['system_info']['serial_number'] == module.params['serial']:
    ...
    # #######################################################################
        ...
        if firewall_element['provisioned'] is False:
            ...
            firewall_element_data["device_info"].append(
                {
                    'root_pwd': module.params['root_pwd'],
                    'serial_number': module.params['serial']
                }
            )
            ...
            # ###########################################################################
            response = rest.post("deviceonboarding/add-device-manual", data=firewall_element_data)
...
def main():
    ...
    # #######################################################################
    argument_spec = SDCloudHelper.ztp_spec()
