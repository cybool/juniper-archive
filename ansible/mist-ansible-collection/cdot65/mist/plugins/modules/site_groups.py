...
def core(module):
    ...
    if response.status_code != 200:
        module.fail_json(
            msg=f"Failed to receive information about the current site groups, here is the response information: {response.info}"
        )
    ...
    if isinstance(site_groups, list):
        ...
        module.fail_json(
            msg=f"The site_groups returned from the API are not in a list format, contant Mist support: {site_groups}"
        )
...
def main():
    ...
    # ### ########################################################################
    argument_spec = MistHelper.site_groups_spec()
