# Ansible Automation Example: JSNAPY BGP Validation
...
This example will show how to validate the status of our BGP neighbors over NETCONF RPCs.
...
```bash
$ docker build -t registry.gitlab.com/cremsburg/juniper-automation-container:validate-bgp files/docker/
```
...
$ docker run -it --rm \
    ...
    registry.gitlab.com/cremsburg/juniper-automation-container:validate-bgp ansible-playbook pb.retrieve.lldp.yaml
```
...
```bash
$ ansible-playbook pb.retrieve.lldp.yaml
```
...
```bash
$ which python
/home/cdot/.cache/pypoetry/virtualenvs/validate-bgp-X7Chj_yD-py3.8/bin/python
```
...
```ini
interpreter_python = /home/cdot/.cache/pypoetry/virtualenvs/validate-bgp-X7Chj_yD-py3.8/bin/python
```
...
Name | Description
---- | -----------
[pb.jsnapy.bgp.rst](files/docs/pb.jsnapy.bgp.rst)|Validate BGP neighbors with JSNAPY
